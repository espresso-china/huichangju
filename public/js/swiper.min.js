...
function t_player(cur){
    $('ul.ul_system > li').eq(n_c_ser).removeClass("nav-cus-"+n_c_ser);
    if(cur==null){
    ...
    $('.inner').animate({marginLeft:(-1 * n_c_ser * 560) + 'px'});//下面内容的动画效果左移动560px；
    $('ul.ul_system > li').eq(n_c_ser).addClass("nav-cus-"+n_c_ser);
}

var n_t = setInterval('t_player()', 5000);//每隔5秒 执行一次

$('ul.ul_system  > li').hover(function(){//当我鼠标悬浮上去的时候
    var self = this;
